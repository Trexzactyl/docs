...
After we've done that, we can make our configuration for Trexzactyl to run.

***

### Create configuration file

> **Note:** Make sure to replace `<domain>` with your own domain in this config file.
...
<VirtualHost *:443>
  ServerName <domain>
  DocumentRoot "/var/www/trexzactyl/public"
  ...
  <Directory "/var/www/trexzactyl/public">
    Require all granted
    AllowOverride all
  </Directory>
...
Then, we'll apply the settings Apache needs to host Trexzactyl.
```bash
sudo a2enmod rewrite
sudo a2enmod ssl
```

Finally, we'll restart Apache in order to bring Trexzactyl online.
```bash
