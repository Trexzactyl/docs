...
Updating Trexzactyl keeps your system stable, secure and allows
your users/admins to experience new features quicker. Use
the following guide below as a reference for updating Trexzactyl.
...
```bash
cd /var/www/trexzactyl # Replace 'trexzactyl' with 'pterodactyl' if you have migrated
php artisan down
```
...
```bash
curl -L https://github.com/trexzactyl/trexzactyl/releases/latest/download/panel.tar.gz | tar -xzv
chmod -R 755 storage/* bootstrap/cache # Set server permissions properly
```

### Update Composer packages

Due to Trexzactyl staying up-to-date using the latest packages, you
will need to update the Composer dependencies which allow Trexzactyl
...
database in order to use the latest Trexzactyl features.
...
webserver so that Trexzactyl can be hosted and accessed properly.
...
> **Tip:** Any issues? Please reach out to us on [Discord](https://discord.gg/Uf59NRyYU9).
